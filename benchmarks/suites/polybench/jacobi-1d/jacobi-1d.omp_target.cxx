...
#pragma omp target data map(to:A[0:n]) map(from:B[0:n]) default(none) firstprivate(tsteps, n)
  {
    for (idx_t t = 0; t < tsteps; t++) {

#pragma omp teams distribute parallel for dist_schedule(static) schedule(static) firstprivate(tsteps, n)
