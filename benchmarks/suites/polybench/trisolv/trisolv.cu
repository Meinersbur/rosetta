...
struct Lij_times_xj : public thrust:: unary_function<real,real>{
  pbsize_t n;thrust::device_ptr<real> L;thrust::device_ptr<real> x; idx_t i;
...
real sum  = thrust::transform_reduce(
  ...
  for (idx_t i = 0; i < n; i++) {
    real sum = 0 ;
#pragma omp parallel for schedule(static) default(none) firstprivate(i,x,L) reduction(+:sum)
    for (idx_t j = 0; j < i; j++)
      sum += L[i][j] * x[j];
    x[i] =  (b[i] - sum) / L[i][i];
  }
