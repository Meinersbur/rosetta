...
#pragma omp target data map(tofrom:y[0:n]) map(to:r[0:n]) map(alloc:z[0:n])  map(tofrom:alpha,beta)
  {
      ...
      for (idx_t k = 1; k < n; k++) {
          ...
          {
#pragma omp target teams distribute parallel for  schedule (static) default(none) firstprivate(k,r,y) reduction(+: sum)  
              for (idx_t i = 0; i < k; i++)
                  sum += r[k - i - 1] * y[i];

#pragma omp target  map(tofrom:alpha,beta) // FIXME: libomp seems to get reference counting from without the map clause
              {
