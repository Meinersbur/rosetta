...
class UserBuilddirMulticonfig(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        invoke.diag('cmake', '-S', cls.srcdir, '-B',cls. builddir,  '-GNinja Multi-Config',  '-DROSETTA_BENCH_FILTER=--filter-include-program-substr=cholesky', '-DROSETTA_PPM_DEFAULT=OFF', '-DROSETTA_PPM_SERIAL=ON', cwd= cls.builddir,onerror=invoke.Invoke.EXCEPTION)
