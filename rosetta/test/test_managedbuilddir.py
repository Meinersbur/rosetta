...
class ManagedBuilddirTests(unittest.TestCase):
    ...
    def test_clean(self):
        ...
        rosetta.driver.driver_main(
            argv=[None, '--clean'], mode=DriverMode.MANAGEDBUILDDIR, rootdir=self.rootdir, srcdir=self.srcdir
        )
...
class ManagedBuilddirDefaultconfig(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        rosetta.driver.driver_main(
            argv=[
                None,
                '--configure',
                '--cmake-def=ROSETTA_BENCH_FILTER=--filter-include-program-exact=idioms.assign',
                '--cmake-def=ROSETTA_PPM_DEFAULT=OFF',
                '--cmake-def=ROSETTA_PPM_SERIAL=ON',
            ],
            mode=DriverMode.MANAGEDBUILDDIR,
            rootdir=cls.rootdir,
            srcdir=cls.srcdir,
        )
    ...
    def test_build(self):
        rosetta.driver.driver_main(
            argv=[None, '--build'], mode=DriverMode.MANAGEDBUILDDIR, rootdir=self.rootdir, srcdir=self.srcdir
        )
        buildlist = list((self.rootdir / 'build').iterdir())
        self.assertEquals(len(buildlist), 2)
        self.assertTrue(
            (self.rootdir / 'build' / 'defaultbuild' / 'benchmarks' / 'Release' / 'idioms.assign.serial').exists()
        )

    def test_probe(self):
        problemsizefile = self.resultsdir / 'proberesult.ini'
        rosetta.driver.driver_main(
            argv=[None, '--problemsizefile-out', problemsizefile, '--limit-walltime=10ms'],
    ...
    def test_verify(self):
        ...
        with contextlib.redirect_stdout(Tee(f, sys.stdout)):
            rosetta.driver.driver_main(
                argv=[None, '--verify'], mode=DriverMode.MANAGEDBUILDDIR, rootdir=self.rootdir, srcdir=self.srcdir
            )
    ...
    def test_bench(self):
        ...
        with contextlib.redirect_stdout(Tee(f, sys.stdout)):
            rosetta.driver.driver_main(
                argv=[None, '--bench'], mode=DriverMode.MANAGEDBUILDDIR, rootdir=self.rootdir, srcdir=self.srcdir
            )
...
class ManagedBuilddirMulticonfig(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        rosetta.driver.driver_main(
            argv=[
                None,
                '--configure',
                '--cmake-def=ROSETTA_BENCH_FILTER=--filter-include-program-exact=idioms.assign --filter-include-program-exact=suites.polybench.cholesky',
                '--cmake-def=ROSETTA_PPM_DEFAULT=OFF',
                '--cmake-def=ROSETTA_PPM_SERIAL=ON',
                "--compiler-arg=O2:-O2",
                "--compiler-arg=O3:-O3",
    ...
    def test_build(self):
        rosetta.driver.driver_main(
            argv=[None, '--build', '--config=O2', '--config=O3'],
            mode=DriverMode.MANAGEDBUILDDIR,
            rootdir=self.rootdir,
            srcdir=self.srcdir,
        )
    ...
    def test_bench(self):
        ...
        with contextlib.redirect_stdout(Tee(f, sys.stdout)):
            rosetta.driver.driver_main(
                argv=[None, '--bench', '--config=O2', '--config=O3'],
                mode=DriverMode.MANAGEDBUILDDIR,
                rootdir=self.rootdir,
                srcdir=self.srcdir,
            )
        ...
        for r in results:
            self.assertTrue(
                r.name.startswith('idioms.assign.') or r.name.startswith('suites.polybench.cholesky.'),
                "Must only run filtered tests",
            )
